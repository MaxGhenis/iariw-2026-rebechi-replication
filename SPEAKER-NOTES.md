...
Chart = our replication's group means. Gap −1.56 (2004) → −2.03 (2009) → −2.55 (2021); pre-period leads +0.46, +0.48, +0.45, +0.37, +0.22; joint pre-test p = 0.40 because SEs are wide. Consistent with the paper: trends spec → ≈ 0 (Fig. 4); SDID groups n.s. Slattery et al. fn. 11 explicitly reject parallel trends in levels of tax rates (and use logs); Gilens et al. use generalized synthetic control, which does not need them. With state-specific trends our panel gives −0.27 (n.s.). Asks: trends spec as co-equal; Rambachan–Roth bounds. Fairness line: RI p = 0.013, wild bootstrap p = 0.030 — not a few-cluster artifact.
